...
import type { AccountActivityMailMerge, ActivityMailMergeOutput } from '@server/api/activity/data/dtos';
      ...
      const response = await emailApi.post(`/account/${account.id}/submit`, {
        ...
        mailMerge: leadsChunk?.[index]?.map(lead => {
          ...
          if (nextAvailableDateTime.isToday() && nextAvailableDateTime.diff(endTime, 'm') > 0) {
            nextAvailableDateTime = nextAvailableDateTime.add(1, 'd').hour(startTime.hour()).minute(startTime.minute());
          }
